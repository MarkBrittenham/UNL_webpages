...
{\bf The dual bundle again:} The existence of a Riemannian metric allows us to answer the 
...
The idea behind this is to push our computations into $\bbr^n$ usng a chart. That is,
...
A smooth map $F:M^n\ra N^m$ is an {\it immersion} if $F_*:T_aM\ra T_{f(a)}N$
...
The proof of this consists, first, of choosing any two charts $h$ and $k$ about $a$ and 
...
$S$ is constant on the $n-k$-dimensional planes where the first $k$ variables are held consant,
...
{\bf Embedded submanifolds:} So far we haven't really talked about the usefulness of immersions, just submersions. 
...
Furthermore, this is the \u{only} smooth structure on $S$ making $\iota$ smooth. To see this, 
...
Vector and covector fields behave well, too. Covector fields are simplest;
...
$(v,w_1+w_2)-(v,w_1)-(v,w_2)$, for $v\in V$ , $w_1,w_2\in W$
...
Applying this to the duals $V^*,W^*$ of two vector
...
Our primary object of study will be the $k$-fold tensor product of the dual $V^*$
...
A section of the bundle $T^k(M)$ is called a covariant tensor field of rank $k$
...
Covariant tensors (or rather, certain covariant tensors) will be the ``correct'' objects
...
The collection of alternating covariant $k$-tensors for a sub-vector space of $T^k(V)$;
...
Every alternating covariant $k$-tensor is therefore a linear combination of 
...
The bundle of alternating $k$-tensors on $M$ is denoted $\Lambda^k(M)$; the collection of smooth sections
...
\ctln{$\sum_i\int_{h_i(U_i)}(h_i^{-1})^*(g_i\omega) = \sum_i\int_{k_j(V_j)}(k_j^{-1})^*(G_j\omega)$.}
...
So by employing a parition of unity, we can formulate the integral of any $n$-form with compact 
...
So now we can focus, essentially, on forms $g_i\omega =\eta$ supported on the interior 
...
Otherwise, it is really the exact same calculation as above, although there are two cases. When $i\neq n$,
...
(b) $\ell=k$ and $D=c\cdot$id for some $c\in \bbr$

(c) $\ell=k+1$ and $D=c\cdot d$ for some $c\in \bbr$, or

(d) $\ell=0$ and $D=c\cdot$``integration'' for some $c\in \bbr$.
